...
then
		rpm -qa | grep aide 2>/dev/null | wc -l > CIS-1.3-FILESYSTEM-INTEGRITY-CHECKING/aide.txt 2>&1
...
then
		...
		echo -e "audit package not installed." >> CIS-4.1-CONFIGURE-SYSTEM-ACCOUNTING/audit-conf.txt 2>&1
		echo -e "audit probably not installed, check the rpm-all.txt and systemctl-all.txt files to verify." >> CIS-4.1-CONFIGURE-SYSTEM-ACCOUNTING/audit-conf.txt 2>&1
fi
...
if command -v rpm &> /dev/null 2>&1
then
        rpm -qa > rpm-all.txt 2>&1
		rpm -qa --last > rpm-last.txt 2>&1
fi
